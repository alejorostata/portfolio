...
export const SkillsSection: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-16 sm:py-20 relative bg-grid-pattern">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex items-center gap-2 mb-8 border-b border-slate-800 pb-3 overflow-x-auto no-scrollbar whitespace-nowrap" role="tablist" aria-label="Skill Categories">
          <button
          ...
          {SKILL_CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                type="button"
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-all outline-none focus:outline-none shrink-0 ${
                  isActive
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
          {displayedSkills.map((category) => (
            <div
              key={category.id}
              className="bg-slate-900/70 border border-slate-800/90 rounded-2xl p-5 sm:p-6 space-y-4 hover:border-slate-700 transition-all duration-300 shadow-lg"
            >
              <div className="flex items-center justify-between border-b border-slate-800/80 pb-3">
                ...
                <span className="text-[11px] sm:text-xs font-mono text-slate-400 bg-slate-950 px-2 py-1 rounded border border-slate-800">
              ...
              <div className="flex flex-wrap gap-1.5 sm:gap-2 pt-1">
                {category.skills.map((skill) => (
                  <div
                    key={skill}
                    className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-950/80 border border-slate-800/80 text-xs font-medium text-slate-200 hover:border-indigo-500/40 hover:text-indigo-300 transition-colors"
                  >
